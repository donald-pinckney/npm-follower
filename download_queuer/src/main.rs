...
fn main() {
    ...
    let mut queued_up_to = internal_state::query_queued_downloads_seq(&mut conn).unwrap_or(0);
    ...
    loop {
        ...
        let changes = change_log::query_changes_after_seq(queued_up_to, PAGE_SIZE, &mut conn);
        ...
        let downloads_to_enqueue: Vec<_> = changes
            .into_iter()
            .flat_map(download_tasks_for_change)
            .collect();

        download_queue::enqueue_downloads(downloads_to_enqueue, &mut conn);
...
trait BetterUnwrap {
    type Wrapped;
    fn unwrap_debug<F>(self, f: F) -> Self::Wrapped
    where
        F: FnOnce() -> String;
}
...
impl<T> BetterUnwrap for Option<T> {
        ...
        F: FnOnce() -> String,
    {
        match self {
            Some(x) => x,
            None => {
                panic!("Expected Some, received None. Reason: {}", f())
            }
        }
    }
}

impl<T, R> BetterUnwrap for Result<T, R>
where
    R: std::fmt::Debug,
{
    type Wrapped = T;
        ...
        F: FnOnce() -> String,
    {
        match self {
            Ok(x) => x,
            Err(e) => {
                panic!("Expected Ok, received Err({:?}). Reason: {}", e, f())
            }
        }
    }
}
...
pub fn download_tasks_for_change(change: Change) -> Vec<download_queue::DownloadTask> {
    ...
    versions
        ...
        .map(|(_v_version, v_data)| {
            ...
            let sig0 = dist.get("signatures").and_then(|sigs| {
                sigs.as_array()
                    .unwrap_debug(seq_debug_print("signatures must be an array"))
                    .first()
                    .map(|s| {
                        s.as_object()
                            .unwrap_debug(seq_debug_print("signature must be an object"))
                    })
            });
