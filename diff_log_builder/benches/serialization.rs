...
const BIG_CHANGE_JSON_STR: &str =
    include_str!("../resources/test_deserialize_change/input/seq_832853_cleaned.json");
const SMALL_CHANGE_JSON_STR: &str =
    include_str!("../resources/test_deserialize_change/input/seq_1166950.json");
...
pub fn bench_parse_big_change(c: &mut Criterion) {
    ...
    c.bench_function("parse test_change_big.json", |b| {
        b.iter(|| deserialize_change(black_box(clone_change(&change))))
    });
}
