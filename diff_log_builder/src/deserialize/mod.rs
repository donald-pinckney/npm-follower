...
fn deserialize_times_normal(j: &mut Map<String, Value>) -> (DateTime<Utc>, DateTime<Utc>, BTreeMap<Result<Semver, String>, DateTime<Utc>>) {
    ...
    let version_times: BTreeMap<Result<Semver, String>, _> = times.into_iter().map(|(v_str, t)| 
...
fn only_keep_ok_version_times(version_times: BTreeMap<Result<Semver, String>, DateTime<Utc>>) -> BTreeMap<Semver, DateTime<Utc>> {
    version_times
        .into_iter()
        .filter_map(|(vr, t)| Some((vr.ok()?, t))).collect()
}
