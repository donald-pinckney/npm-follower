...
CREATE OR REPLACE VIEW metadata_analysis.version_prod_dep_matches_version AS
WITH 
disjunct_unnest AS (
  SELECT 
    *, 
  ...
  FROM (
    ...
    WHERE dst_package_id_if_exists IS NOT NULL AND (spec).dep_type = 'range'
  ) t
),
...
conjunct_eval AS (
  ...
  ON v.package_id = t.dst_package_id_if_exists --AND metadata_analysis.version_comp_exclude_betas(v.semver, t.term)
...
SELECT v.id as src_v, dep_match_v.v_id as dst_v
FROM (SELECT id, unnest(prod_dependencies) as src_dep_id from versions) v
INNER JOIN dep_version_match_rel dep_match_v
ON v.src_dep_id = dep_match_v.dep_id
;


-- SELECT * FROM metadata_analysis.version_prod_dep_matches_version
-- WHERE dst_v = 19989635
