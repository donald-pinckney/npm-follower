...
CREATE TABLE tarball_analysis.diffs_to_compute AS
SELECT u.*,
...
ALTER TABLE tarball_analysis.diffs_to_compute
ADD PRIMARY KEY (from_id, to_id);

DELETE FROM tarball_analysis.diffs_to_compute WHERE ROW(from_id, to_id) IN (SELECT from_id, to_id FROM tarball_analysis.diff_analysis);

GRANT ALL ON tarball_analysis.diffs_to_compute TO pinckney;
GRANT ALL ON tarball_analysis.diffs_to_compute TO federico;
GRANT SELECT ON tarball_analysis.diffs_to_compute TO data_analyzer;

ANALYZE tarball_analysis.diffs_to_compute;