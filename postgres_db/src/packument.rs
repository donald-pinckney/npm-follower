...
impl PackageOnlyPackument {
    pub fn serialize_and_hash(&self) -> (Value, String, usize) {
        ...
        let s = serde_json::to_vec(self).unwrap();
        let n_bytes = s.len();
        let mut hasher = Sha256::new();
        hasher.update(s);
        let result = hasher.finalize();

        // TODO[perf]: Return a serialized JSON value here, and pass it through to DB inserts later so we can save on serailization cost.
        (Value::Null, format!("{:x}", result), n_bytes)
    }
