...
fn sanitize_null_escapes(s: &str) -> String {
    ...
    for c in s.chars() {
        state = match (state, c) {
            ...
            (S2, '0') => S3,
            (S2, x) => { sanitized.push_str("\\u"); sanitized.push(x); S0 },
            (S3, '0') => S4,
            (S3, x) => { sanitized.push_str("\\u0"); sanitized.push(x); S0 },
            (S4, '0') => S5,
            (S4, x) => { sanitized.push_str("\\u00"); sanitized.push(x); S0 },
            (S5, '0') => { sanitized.push_str("[NULL]"); S0 },
            (S5, x) => { sanitized.push_str("\\u000"); sanitized.push(x); S0 }
        };
    }
...
mod tests {
    ...
    fn sanitize_1_bad_end() {
        assert_eq!(sanitize_null_escapes(r"bad:\u0000"), "bad:[NULL]".to_string());
    }

    #[test]
    fn sanitize_1_bad_start() {
        assert_eq!(sanitize_null_escapes(r"\u0000:bad"), "[NULL]:bad".to_string());
    }

    #[test]
    fn sanitize_2_bad() {
        assert_eq!(sanitize_null_escapes(r"\u0000\u0000"), "[NULL][NULL]".to_string());
    }

    #[test]
    fn sanitize_4_bad() {
        assert_eq!(sanitize_null_escapes(r"\u0000\u0000\u0000\u0000"), "[NULL][NULL][NULL][NULL]".to_string());
    }

    #[test]
    fn sanitize_2_bad_separated() {
        assert_eq!(sanitize_null_escapes(r"A\u0000A\u0000A"), "A[NULL]A[NULL]A".to_string());
    }

    #[test]
    fn sanitize_4_bad_separated() {
        assert_eq!(sanitize_null_escapes(r"A\u0000A\u0000A\u0000A\u0000A"), "A[NULL]A[NULL]A[NULL]A[NULL]A".to_string());
    }

    #[test]
    fn sanitize_bad_extra_backslack() {
        assert_eq!(sanitize_null_escapes(r"start \\u0000 end"), r"start \\u0000 end".to_string());
    }
}
