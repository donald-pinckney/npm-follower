...
pub fn check_no_concurrent_processes(name: &str) {
    ...
    let mut pids: HashSet<_> = String::from_utf8(pidof_output.stdout)
        .unwrap()
        .split_whitespace()
    ...
    if !pids.is_empty() {
        println!(
            "Cannot run download_queuer concurrently. Already running with PIDs: {}",
            pids.iter()
                .map(|pid| format!("{}", pid))
                .collect::<Vec<_>>()
                .join(" ")
...
pub trait RemoveInto {
    fn remove_key<T>(&mut self, key: &'static str) -> Option<Result<T, serde_json::Error>>
    where
