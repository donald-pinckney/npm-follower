...
pub struct Packument {
    ...
    pub versions: Vec<VersionPackument>,
...
impl TryFrom<Map<String, Value>> for Packument {
    type Error = String;

    fn try_from(mut j: Map<String, Value>) -> Result<Self, Self::Error> {
        ...
        let version_packuments_map = j
            .remove("versions")
            .map(|x| utils::unwrap_object(x).unwrap())
            .unwrap_or_default();
            
        let version_packuments = version_packuments_map.into_iter()
            .map(|(v_str, blob)| {
                let v = Version::parse(v_str).unwrap();
                let t = version_times.get(&v).unwrap().clone();
                return process_version(v, t, utils::unwrap_object(blob).unwrap())
            }).collect();
...
fn process_version(v: Version, t: DateTime<Utc>, mut version_blob: serde_json::Map<String, Value>) -> VersionPackument {
    ...
    let shasum = utils::unwrap_string(dist.remove("shasum").unwrap()).expect(&format!("Expected a string. Blob = {:?}", version_blob));
