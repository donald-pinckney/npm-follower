...
impl SqlInsertable for Package {
  const INSERT_TEMPLATE: &'static str = r"
    INSERT INTO `package` 
      (`name`, `latest_version`, `created`, `modified`, `other_dist_tags`) VALUES 
      (?1, ?2, ?3, ?4, ?5)
  ";
  fn params(&self) -> Vec<&dyn ToSql> { 
    ...
    // &stuff[..]
    // let 
    vec![&self.name, &self.latest_version, &self.created, &self.modified, &self.other_dist_tags]
  }
}
