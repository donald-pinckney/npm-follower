...
def check_valid(rf_and_local):
    ...
    if rf.lfs is not None:
        ...
        backup_sha_bytes = b"__danger__ __bug__ [try 0] should be sha = " + disk_sha_hex.encode()
        backup_sha_hex = hashlib.sha256(backup_sha_bytes).hexdigest()

        if disk_sha_hex != hf_sha_hex:
            print(f"({dst_path}): On-disk SHA of {disk_sha_hex} does not match HF SHA of {hf_sha_hex}")
            print(f"({dst_path}): Checking backup SHA instead: sha256({backup_sha_bytes}) = {backup_sha_hex}")
            if backup_sha_hex != hf_sha_hex:
                print(f"({dst_path}): Backup SHA is also wrong. Need to re-download")
                return (src_url, rf.rfilename)
...
def main():
    ...
    rf_and_locals = [(r, os.path.join(dst_dir, r.rfilename)) for r in repo_files]
    to_redownload = process_map(check_valid, rf_and_locals, max_workers=8, chunksize=4)
